...
def compute_bleu_bert(Xbatch, pred_batch, bert_tokenizer, eos_token_idx):
    Xtext = [bert_tokenizer.convert_ids_to_tokens(X, skip_special_tokens=True)
             for X in Xbatch]
    pred_text = [bert_tokenizer.convert_ids_to_tokens(X, skip_special_tokens=True)  # noqa
                 for X in pred_batch]
...
def trainstep(model, optimizer, dataloader, params, epoch, idx2word,
    ...
    for (i, batch) in enumerate(dataloader):
        in_Xbatch = batch["noisy"]
        for key in in_Xbatch:
            in_Xbatch[key] = in_Xbatch[key].to(model.device)
        target_Xbatch = batch["orig"]["input_ids"].to(model.device)
        Ybatch = batch["labels"]
        batch_ids = batch["ids"]
        ...
        output = model(
            in_Xbatch, teacher_forcing_prob=params["teacher_forcing_prob"])
        lengths = in_Xbatch["attention_mask"].sum(axis=1)
        ...
        # output_prime = model(
        #         x_prime, teacher_forcing_prob=params["teacher_forcing_prob"])
        ...
        # Measure self-BLEU
        bleu = compute_bleu_bert(target_Xbatch, x_prime, model.tokenizer,
                                 model.eos_token_idx)
        bleus.append(bleu)
...
def run(params_file, verbose=False):
    ...
    train_data = data_utils.DenoisingTextDatasetBert(
            noisy_train_sents, train_sents, train_labs, train_ids,
            "bert-base-uncased", label_encoders)
    ...
    if params["validate"] is True:
        dev_data = data_utils.DenoisingTextDatasetBert(
    ...
    sos_token_idx = train_data.tokenizer.cls_token_id
    eos_token_idx = train_data.tokenizer.sep_token_id
    label_dims_dict = train_data.y_dims
    vae = model_bert.build_vae(
        params, len(vocab), emb_matrix, label_dims_dict, DEVICE,
